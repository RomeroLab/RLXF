...
def generate_heatmap(WT, probabilities, model_identifier, sequence, filepath, ep, version, tokenizer):
    ...
    for pos, token in enumerate(sequence):  
        ...
        if token_id in all_token_ids:  # Check if the token exists in the token list
            ...
            plt.scatter(pos + 0.5, token_index + 0.5, color=dot_color, s=5)  # Adjust dot size as needed
...
def generate_high_confidence_mutant_sequences(WT, probabilities, tokenizer, model_identifier, num_designs=5, threshold=0.9, num_mutations=10):
    """
    ...
    for pos, wt_res in enumerate(WT):
        ...
        if high_conf_tokens:
            best_mutation = max(high_conf_tokens, key=lambda x: x[1])
            mutation_candidates.append((pos, best_mutation[0], best_mutation[1]))  # (pos, token, prob)
    ...
    # Sort by probability and select top mutations
    mutation_candidates.sort(key=lambda x: x[2], reverse=True)
    selected_mutations = mutation_candidates[:num_mutations]

    # Mutate the WT sequence
    mutated_seq = list(WT)
    for pos, mut_token, _ in selected_mutations:
        mutated_seq[pos] = mut_token
